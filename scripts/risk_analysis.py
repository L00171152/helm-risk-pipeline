...
# Optional legacy hard gates (kept for compatibility; can be removed if you want only level-based gating)
FAIL_ON_HIGH = int(os.environ.get("FAIL_ON_HIGH", "0"))  # e.g., 100
FAIL_ON_CRIT = int(os.environ.get("FAIL_ON_CRIT", "0"))  # e.g., 10

# How many top CVEs to list per image in details
TOP_FINDINGS_PER_IMAGE = int(os.environ.get("TOP_FINDINGS_PER_IMAGE", "5"))

# ---- Risk level policy (tunable via env) ----
# Default:
# - High  : CRITICAL >= 1  OR HIGH >= 50  OR risk_score >= 100
# - Medium: CRITICAL == 0 AND (HIGH >= 10 OR risk_score >= 20)
# - Low   : otherwise
CRIT_HIGH_CUTOFF  = int(os.environ.get("CRIT_HIGH_CUTOFF", "1"))
HIGH_HIGH_CUTOFF  = int(os.environ.get("HIGH_HIGH_CUTOFF", "50"))
SCORE_HIGH_CUTOFF = int(os.environ.get("SCORE_HIGH_CUTOFF", "100"))

HIGH_MED_CUTOFF   = int(os.environ.get("HIGH_MED_CUTOFF", "10"))
SCORE_MED_CUTOFF  = int(os.environ.get("SCORE_MED_CUTOFF", "20"))
...
def classify_level(tot: Counter, score: int) -> str:
    if tot["CRITICAL"] >= CRIT_HIGH_CUTOFF or tot["HIGH"] >= HIGH_HIGH_CUTOFF or score >= SCORE_HIGH_CUTOFF:
        return "High"
    if tot["CRITICAL"] == 0 and (tot["HIGH"] >= HIGH_MED_CUTOFF or score >= SCORE_MED_CUTOFF):
        return "Medium"
    return "Low"
...
def main():
    ...
    # Overall numeric score (simple heuristic)
    risk_score = 3 * total["CRITICAL"] + total["HIGH"]
    risk_level = classify_level(total, risk_score)
    ...
    if top_images:
        ...
        for img, cnt in top_images:
            lines.append(
                f"- `{img}` → CRIT {cnt['CRITICAL']}, HIGH {cnt['HIGH']}, "
    ...
    for img, cnt in sorted(
        per_image.items(),
        key=lambda kv: (kv[1]["CRITICAL"], kv[1]["HIGH"]),
        reverse=True,
    ):
        r = 3 * cnt["CRITICAL"] + cnt["HIGH"]
        rows.append(
            [
                img,
                str(cnt["CRITICAL"]),
    ...
    for img in sorted(per_image_vulns.keys()):
        top = sorted(per_image_vulns[img], key=lambda x: x[0], reverse=True)[
            :TOP_FINDINGS_PER_IMAGE
        ]
        det.append(f"### {img}")
        if not top:
    ...
    # ---- gating: fail CI if overall risk is High ----
    if risk_level == "High":
        print("::error ::Overall Risk Level is High – blocking PR")
        sys.exit(1)

    # (Optional) legacy gates: still supported if you set env vars
